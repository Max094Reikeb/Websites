...
if (isset($_POST['username']) && isset($_POST['email']) && isset($_POST['password']) && isset($_POST['confirmation'])) {
	...
	if ($password == $confirmation) {
		...
		if ($select->rowCount() == 0) {
			...
			$insert = $bdd->prepare('INSERT INTO users (username, email, role, password, salt) VALUES (:username, :email, 1, :password, :salt)');
			$inserted = $insert->execute(array(
				'username' => $username,
				'email' => $email,
				'password' => $password,
				'salt' => $salt
...
<head>
	...
	<title>Inscription - Max094 Reikeb</title>
